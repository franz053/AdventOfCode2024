...
int search(vector<int> &topographicMap, int width, int pos, set<int> &visited) {
    if (topographicMap[pos] == 9 && !visited.contains(pos)) {
        visited.insert(pos);
        return 1;
    }
    int ret = 0;
    ...
    if (pos - width >= 0 && topographicMap[pos - width] == topographicMap[pos] + 1) {
        ret += search(topographicMap, width, pos - width, visited);
    }
    //left
    if (pos % width != 0 && pos - 1 >= 0 && topographicMap[pos - 1] == topographicMap[pos] + 1) {
        ret += search(topographicMap, width, pos - 1, visited);
    }
    //right
...
int main() {
    ...
    for (int pos = 0; pos < topographicMap.size(); ++pos) {
        set<int> visited;
        if (topographicMap[pos] == 0) result += search(topographicMap, inString.length(), pos, visited);
    }
