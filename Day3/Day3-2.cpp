...
#define INPUT_PATH "C:\\Users\\krempl\\Downloads\\Day3\\input.txt"

int main(){
    std::ifstream InFile(INPUT_PATH);
    std::string currentLine;
    const std::regex expression("mul\\([0-9]{1,3},[0-9]{1,3}\\)|do(n't)?\\(\\)");
    int result = 0;
    bool enabled = true;
    while (std::getline(InFile, currentLine)) {
        ...
        while(std::regex_search(currentLine,match,expression)){
            ...
            if(found == "do()") enabled = true;
            else if(found == "don't()") enabled = false;
            else if(enabled){
